using System;
using System.Collections.Generic;


public class CircularFiniteStateMachine<EState> : State
where EState : Enum
{
    //private EState                      _currentStateID;
    //private FiniteStateMachine<EState>  _fsm = null;

    //public int StateCount { get { return _fsm.StateCount; } }

    //public CircularFiniteStateMachine() { }

    //public void InitializeOrderedStates(params KeyValuePair<EState, State>[] newStates)
    //{
    //    _fsm.InitializeStates(newStates);
    //    _fsm.SetDefaultState();
    //}

    //#region State Interface
    //public override void Enter()
    //{
    //    _fsm.Enter();
    //}

    //public override void Exit()
    //{
    //    _fsm.Exit();
    //}

    //public override void LogicUpdate()
    //{
    //    _fsm.LogicUpdate();
    //}

    //public override void PhysicsUpdate()
    //{
    //    _fsm.PhysicsUpdate();
    //}
    //#endregion State Interface

    //public void ChangeNextState()
    //{
    //    _currentStateID = (_currentStateID + 1) % StateCount;
    //    _fsm.ChangeState(_currentStateID);
    //}

    //public void ChangeDefaultState()
    //{
    //    _currentStateID = 0;
    //    _fsm.ChangeState(_currentStateID);
    //}
}
